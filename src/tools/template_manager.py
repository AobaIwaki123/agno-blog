...
*Source: {source_url}*""",
                ...
                "is_active": True,
                "variables": [
                    "title", "introduction", "main_content", "key_points",
                    "conclusion", "publish_date", "tags", "source_url"
                ],
                "feedback_history": []
            }
            ...
            for template_file in self.templates_dir.glob("*.json"):
                ...
                if template_data:
                    ...
                    templates.append({
                        ...
                        "feedback_score": template_data.get("feedback_score", 5.0),
                        "is_active": template_data.get("is_active", True),
                        "updated_at": template_data.get("updated_at", "")
                    })
            
            # Sort by usage count and feedback score
            templates.sort(key=lambda x: (x["usage_count"], x["feedback_score"]), reverse=True)
    ...
    def create_template(self, name: str, content: str, description: str = "", 
        ...
        try:
            ...
            if self.save_template(template_id, template_data):
                logger.info(f"Created new template: {template_id} ({name})")
                return template_id
            else:
    ...
    def update_template_from_feedback(self, template_id: str, feedback: str, 
        ...
        try:
            ...
            proposed_changes = self._generate_template_changes(
            ...
            if proposed_changes and user_confirmation:
                updated_template = self._apply_template_changes(
                ...
                updated_template["feedback_history"].append(feedback_entry)
                
                # Update version
                current_version = updated_template.get("version", "1.0.0")
                version_parts = current_version.split(".")
                version_parts[-1] = str(int(version_parts[-1]) + 1)
                updated_template["version"] = ".".join(version_parts)
                
                # Save updated template
                if self.save_template(template_id, updated_template):
                    return {
                        "status": "success",
                    ...
                    return {"status": "error", "message": "Failed to save updated template"}
            
            return {"status": "no_changes", "message": "No changes to apply"}
            
        except Exception as e:
            logger.error(f"Error updating template from feedback: {str(e)}")
    ...
    def _analyze_feedback(self, feedback: str) -> Dict[str, Any]:
        ...
        # Detect sentiment
        positive_words = ["good", "great", "excellent", "perfect", "like", "love", "better"]
        negative_words = ["bad", "poor", "terrible", "hate", "dislike", "wrong", "issue"]
        ...
        sections = ["title", "introduction", "conclusion", "content", "summary"]
        for section in sections:
            if section in feedback_lower:
                analysis["sections_mentioned"].append(section)
        
        return analysis
    
    def _generate_template_changes(self, template_data: Dict[str, Any], 
        ...
        for section in feedback_analysis.get("sections_mentioned", []):
            changes.append({
    ...
    def _apply_template_changes(self, template_data: Dict[str, Any], 
        ...
        for change in changes:
            if change["type"] == "content_adjustment":
                ...
                if "## Additional Information" not in content:
                    ...
                    if "additional_info" not in updated_template.get("variables", []):
                        updated_template["variables"].append("additional_info")
    ...
    def increment_usage_count(self, template_id: str) -> bool:
        ...
        try:
            ...
            return self.save_template(template_id, template_data)
            
        except Exception as e:
            logger.error(f"Error incrementing usage count: {str(e)}")
            return False
    
    def update_feedback_score(self, template_id: str, score: float) -> bool:
        """
        ...
        try:
            ...
            current_score = template_data.get("feedback_score", 5.0)
            usage_count = template_data.get("usage_count", 1)
            
            # Weighted average based on usage
            new_score = ((current_score * usage_count) + score) / (usage_count + 1)
            template_data["feedback_score"] = round(new_score, 2)
            
            return self.save_template(template_id, template_data)
            
        except Exception as e:
            logger.error(f"Error updating feedback score: {str(e)}")
            return False
...
if __name__ == "__main__":
    ...
    template_manager = TemplateManagementTools(templates_dir="test_templates")
    ...
    result = template_manager.update_template_from_feedback("default", feedback, False)
