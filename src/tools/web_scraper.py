...
class WebScrapingTools:
    ...
    def extract_content(self, url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(clean_content) > self.max_content_length:
                clean_content = (
                    clean_content[: self.max_content_length]
                    + "..."
                )
                logger.warning(
                    f"Content truncated to {self.max_content_length} characters"
                )
            ...
            logger.error(
                f"HTTP error extracting from {url}: {error_msg}"
            )
            return {
                "status": "error",
                "message": error_msg,
            ...
            logger.error(
                f"Request error extracting from {url}: {error_msg}"
            )
            return {
                "status": "error",
                "message": error_msg,
            ...
            logger.error(
                f"Unexpected error extracting from {url}: {error_msg}"
            )
            return {
                "status": "error",
                "message": error_msg,
    ...
    def _extract_metadata(
        ...
        if "content" in locals():
            ...
            metadata["estimated_reading_time"] = max(
                1, words // 200
