...
class URLProcessorAgent:
    ...
    def __init__(self, db: SqliteDb):
        ...
        def analyze_extracted_content(
            ...
            """
            return self._analyze_extracted_content_impl(
                content, title
            )
    ...
    def _extract_url_content_impl(
        ...
        try:
            logger.info(
                f"URL Processor Agent: Extracting content from {url}"
            )

            # Use web scraper to extract content
            extraction_result = (
                self.web_scraper.extract_content(url)
            )
            ...
            clean_content = (
                self.content_processor.clean_content(
                    raw_content
                )
            )
            content_analysis = self.content_processor.analyze_content_structure(
                clean_content
            )
            keywords = (
                self.content_processor.extract_keywords(
                    clean_content, max_keywords=10
                )
            )
            summary = (
                self.content_processor.generate_summary(
                    clean_content, max_length=300
                )
            )
            entities = (
                self.content_processor.extract_entities(
                    clean_content
                )
            )
            ...
            logger.info(
                f"Successfully processed content from {url}"
            )
            return result

        except Exception as e:
            logger.error(
                f"Error in URL extraction tool: {str(e)}"
            )
            return {
                "status": "error",
    ...
    def _analyze_extracted_content_impl(
        ...
        try:
            logger.info(
                "URL Processor Agent: Performing content analysis"
            )
            ...
            structure_analysis = self.content_processor.analyze_content_structure(
                content
            )
            keywords = (
                self.content_processor.extract_keywords(
                    content, max_keywords=15
                )
            )
            entities = (
                self.content_processor.extract_entities(
            ...
            sections = (
                self.content_processor.split_into_sections(
                    content, max_section_length=800
                )
            )
            ...
            short_summary = (
                self.content_processor.generate_summary(
                    content, max_length=150, sentences=2
                )
            )
            medium_summary = (
                self.content_processor.generate_summary(
                    content, max_length=300, sentences=4
                )
            )

            # Content quality indicators
            quality_indicators = {
                "has_sufficient_content": structure_analysis[
                    "word_count"
                ]
                >= 100,
                "good_readability": structure_analysis[
                    "avg_words_per_sentence"
                ]
                <= 25,
                "well_structured": structure_analysis[
                    "paragraph_count"
                ]
                >= 2,
            ...
            result = {
                ...
                "content_hash": self.content_processor.generate_content_hash(
                    content
                ),
            ...
            logger.error(
                f"Error in content analysis tool: {str(e)}"
            )
            return {
                "status": "error",
    ...
    async def process_url(
        self, url: str, additional_context: str = ""
    ) -> Dict[str, Any]:
        """
        ...
        try:
            logger.info(
                f"URL Processor Agent: Starting to process URL: {url}"
            )
            ...
            logger.info(
                f"URL Processor Agent: Completed processing URL: {url}"
            )
    ...
    def extract_links_from_url(
        ...
        try:
            ...
            logger.error(
                f"Error extracting links from {url}: {str(e)}"
            )
            return []
    ...
    def get_page_images(
        ...
        try:
            ...
            logger.error(
                f"Error extracting images from {url}: {str(e)}"
            )
            return []
    ...
    def batch_process_urls(
        ...
        for url in urls:
            try:
                result = self.web_scraper.extract_content(
                    url
                )
...
if __name__ == "__main__":
    ...
    async def test_url_processor():
        ...
        if validation.get("valid", False):
            ...
            print(
                f"URL processing result: {result['status']}"
            )

            if result["status"] == "success":
                print(
                    f"Agent response: {result['agent_response'][:200]}..."
                )
