...
class ContentGeneratorAgent:
    ...
    def __init__(self, db: SqliteDb, knowledge: Optional[Knowledge] = None):
    ...
    def _create_blog_generation_tool(self):
        ...
        def generate_blog_post(extracted_content: Dict[str, Any], template_id: str = "default", 
            ...
            """
            try:
                logger.info(f"Content Generator: Generating blog post using template {template_id}")
                ...
                target_audience = style_prefs.get("target_audience", "general")
                content_length = style_prefs.get("content_length", "medium")
                
                # Generate blog post components
                blog_components = self._generate_blog_components(
                    content, title, summary, keywords, tone, target_audience, content_length
                )
                
                # Render template with generated components
                template_variables = {
                    "title": blog_components["title"],
                    "introduction": blog_components["introduction"],
                    "main_content": blog_components["main_content"],
                ...
                for var in template_data.get("variables", []):
                    if var not in template_variables:
                        template_variables[var] = blog_components.get(var, f"[{var}]")
                
                # Render the final blog post
                blog_content = self.template_manager.render_template(template_id, template_variables)
                ...
                metadata = {
                    ...
                    "source_content_hash": self.content_processor.generate_content_hash(content),
                    "word_count": len(blog_content.split()),
                    "estimated_reading_time": max(1, len(blog_content.split()) // 200)
                }
    ...
    def _create_content_enhancement_tool(self):
        ...
        def enhance_blog_content(blog_content: str, enhancement_type: str = "readability") -> Dict[str, Any]:
            ...
            """
            try:
                logger.info(f"Content Generator: Enhancing content for {enhancement_type}")
                ...
                if enhancement_type == "readability":
                    ...
                    changes_made.append("Improved readability and structure")
                
                elif enhancement_type == "seo":
                    # SEO optimization
                    enhanced_content = self._enhance_seo(enhanced_content)
                    changes_made.append("Optimized for SEO")
                
                elif enhancement_type == "engagement":
                    # Improve engagement
                    enhanced_content = self._enhance_engagement(enhanced_content)
                    changes_made.append("Enhanced for reader engagement")
    ...
    def _create_seo_optimization_tool(self):
        ...
        def optimize_for_seo(content: str, target_keywords: List[str] = None) -> Dict[str, Any]:
            ...
            """
            try:
                logger.info("Content Generator: Optimizing content for SEO")
                
                # Extract current keywords if none provided
                if not target_keywords:
                    keywords_with_freq = self.content_processor.extract_keywords(content, max_keywords=10)
                    target_keywords = [kw[0] for kw in keywords_with_freq[:5]]
                ...
                seo_analysis = {
                    ...
                    "title_suggestions": self._generate_title_suggestions(content, target_keywords),
                    "header_structure": self._analyze_header_structure(content),
                    "internal_linking_opportunities": self._find_linking_opportunities(content),
                    "content_length": len(content.split()),
                    "readability_score": self._calculate_readability_score(content)
                }
                
                # Generate optimized content
                optimized_content = self._apply_seo_optimizations(content, target_keywords)
    ...
    def _generate_blog_components(self, content: str, title: str, summary: str, 
                                ...
                                content_length: str) -> Dict[str, str]:
        """Generate individual components for the blog post."""
        
        # Analyze content structure
        sections = self.content_processor.split_into_sections(content, max_section_length=500)
        
        components = {}
        
        # Generate improved title
        components["title"] = self._improve_title(title, keywords, tone)
        
        # Generate introduction
        components["introduction"] = self._generate_introduction(summary or content[:300], tone, target_audience)
        
        # Process main content
        components["main_content"] = self._process_main_content(sections, tone, content_length)
        
        # Extract key points
        components["key_points"] = self._extract_key_points(content, keywords)
        
        # Generate conclusion
        components["conclusion"] = self._generate_conclusion(content, tone)
        
        return components
    
    def _improve_title(self, original_title: str, keywords: List[str], tone: str) -> str:
    ...
    def _generate_introduction(self, content_preview: str, tone: str, target_audience: str) -> str:
    ...
    def _process_main_content(self, sections: List[Dict[str, Any]], tone: str, content_length: str) -> str:
        ...
        for i, section in enumerate(sections):
            ...
            if len(sections) > 1 and content_length in ["long", "detailed"]:
                processed_content += f"\n\n### Section {i+1}\n\n"
    ...
    def _extract_key_points(self, content: str, keywords: List[str]) -> str:
        ...
        for sentence in sentences:
            ...
            if len(sentence) > 50 and any(keyword in sentence.lower() for keyword in keywords):
                key_sentences.append(f"• {sentence}")
                if len(key_sentences) >= 3:
                    break
        
        return "\n".join(key_sentences) if key_sentences else "• Key insights from the content\n• Important takeaways\n• Actionable recommendations"
    
    def _generate_conclusion(self, content: str, tone: str) -> str:
    ...
    def _enhance_seo(self, content: str) -> str:
        ...
        for line in lines:
            line = line.strip()
            if line and not line.startswith('#') and len(line) < 100 and line.isupper():
    ...
    def _generate_meta_description(self, content: str) -> str:
        ...
        return self.content_processor.generate_summary(content, max_length=160, sentences=2)
    
    def _generate_title_suggestions(self, content: str, keywords: List[str]) -> List[str]:
        """Generate alternative title suggestions."""
        suggestions = []
        
        if keywords:
            suggestions.append(f"Complete Guide to {keywords[0].title()}")
            suggestions.append(f"Understanding {keywords[0].title()}: Key Insights")
            suggestions.append(f"Everything You Need to Know About {keywords[0].title()}")
    ...
    def _analyze_header_structure(self, content: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "has_proper_structure": headers["h1"] == 1 and headers["h2"] > 0
        }
    
    def _find_linking_opportunities(self, content: str) -> List[str]:
        """Find opportunities for internal linking."""
        # Simple implementation - look for common topics
        opportunities = []
        
        common_topics = ["tutorial", "guide", "introduction", "overview", "comparison"]
        content_lower = content.lower()
        
        for topic in common_topics:
            if topic in content_lower:
                opportunities.append(f"Link to related {topic} content")
    ...
    def _apply_seo_optimizations(self, content: str, keywords: List[str]) -> str:
        ...
        if keywords and keywords[0].lower() not in content.lower():
            ...
            if paragraphs:
                ...
                paragraphs[0] = f"{first_para} This relates to {keywords[0]} and its applications."
                optimized = '\n\n'.join(paragraphs)
        
        return optimized
    
    def _generate_seo_recommendations(self, seo_analysis: Dict[str, Any]) -> List[str]:
        ...
        if not headers.get("has_proper_structure", False):
            recommendations.append("Improve header structure with proper H1 and H2 tags")
        
        # Check keyword density
        keyword_density = seo_analysis.get("keyword_density", {})
        for keyword, density in keyword_density.items():
            if density < 0.5:
                recommendations.append(f"Consider using '{keyword}' more frequently (current density: {density:.1f}%)")
            elif density > 3.0:
                recommendations.append(f"Reduce usage of '{keyword}' to avoid keyword stuffing (current density: {density:.1f}%)")
    ...
    async def generate_blog_post(self, extracted_content: Dict[str, Any], 
        ...
        """
        try:
            logger.info(f"Content Generator Agent: Generating blog post with template {template_id}")
            ...
            logger.info("Content Generator Agent: Blog post generation completed")
...
if __name__ == "__main__":
    ...
    async def test_content_generator():
        ...
        result = await content_generator.generate_blog_post(extracted_content, "default")
