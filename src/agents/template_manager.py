...
class TemplateManagerAgent:
    ...
    def _create_feedback_analysis_tool(self):
        ...
        def analyze_user_feedback(
            ...
            try:
                logger.info(
                    f"Template Manager: Analyzing feedback for template {template_id}"
                )
                ...
                logger.info(
                    f"Feedback analysis completed for template {template_id}"
                )
                return result

            except Exception as e:
                logger.error(
                    f"Error analyzing feedback: {str(e)}"
                )
                return {
                    "status": "error",
    ...
    def _create_template_update_tool(self):
        ...
        def update_template_from_feedback(
            ...
            try:
                logger.info(
                    f"Template Manager: Updating template {template_id}"
                )
                ...
                if not user_confirmation:
                    return {
                        ...
                        "proposed_changes": feedback_analysis.get(
                            "improvement_suggestions", []
                        ),
                        "impact_assessment": feedback_analysis.get(
                            "impact_assessment", {}
                        ),
                    }
                ...
                # Apply improvements
                updated_template = (
                    self._apply_template_improvements(
                        template_data,
                ...
                if self.template_manager.save_template(
                    ...
                    return {
                        ...
                        "changes_applied": feedback_analysis.get(
                            "improvement_suggestions", []
                        ),
    ...
    def _create_template_validation_tool(self):
        ...
        def validate_template_structure(
            ...
            try:
                logger.info(
                    "Template Manager: Validating template structure"
                )
                ...
                if (
                    "{content}" not in template_content
                    and "{main_content}"
                ...
                validation_results["found_variables"] = (
                    found_variables
                )
                validation_results["variable_count"] = len(
                    found_variables
                )

                logger.info(
                    f"Template validation completed. Valid: {validation_results['is_valid']}"
                )
                return validation_results

            except Exception as e:
                logger.error(
                    f"Error validating template: {str(e)}"
                )
                return {
                    "is_valid": False,
                    "issues": [
    ...
    def _perform_detailed_feedback_analysis(
        self,
        feedback: str,
        ...
        if positive_score > negative_score:
            ...
            analysis["severity"] = (
                "medium" if negative_score > 2 else "low"
            )
        ...
        for issue_type, patterns in issue_patterns.items():
            if any(
                pattern in feedback_lower
                for pattern in patterns
            ):
                analysis["specific_issues"].append(
                    issue_type
                )
                analysis["feedback_categories"].append(
                    issue_type
                )
        ...
        for section, keywords in template_sections.items():
            if any(
                ...
                analysis["affected_sections"].append(
                    section
                )
    ...
    def _assess_change_impact(
        ...
        high_impact_count = sum(
            ...
            if s.get("type") in high_impact_types
        )
        medium_impact_count = sum(
            1
            for s in suggestions
            if s.get("type") in medium_impact_types
        )

        if high_impact_count > 0:
            impact_assessment["overall_impact"] = "high"
            impact_assessment["risk_level"] = "medium"
            impact_assessment["estimated_effort"] = (
                "significant"
            )
            impact_assessment["backward_compatibility"] = (
                False
            )
        ...
        for suggestion in suggestions:
            if suggestion.get("section"):
                impact_assessment[
                    "affected_components"
                ].append(suggestion["section"])
            if suggestion.get("type"):
                impact_assessment[
                    "affected_components"
                ].append(suggestion["type"])

        impact_assessment["affected_components"] = list(
            set(impact_assessment["affected_components"])
        )

        return impact_assessment

    def _apply_template_improvements(
        self,
        template_data: Dict[str, Any],
        suggestions: List[Dict[str, Any]],
    ) -> Dict[str, Any]:
        ...
        for suggestion in suggestions:
            ...
            if suggestion_type == "clarity_improvement":
                ...
                if "word count" not in content.lower():
                    ...
                    changes_applied.append(
                        "Added length guidelines"
                    )

            elif (
                suggestion_type == "structural_improvement"
            ):
        ...
        updated_template["changes_applied"] = (
            changes_applied
        )
    ...
    def _update_template_metadata(
        ...
        feedback_entry = {
            ...
            "changes_applied": template_data.get(
                "changes_applied", []
            ),
        ...
        template_data["feedback_history"].append(
            feedback_entry
        )
    ...
    async def process_feedback(
        ...
        try:
            logger.info(
                f"Template Manager Agent: Processing feedback for template {template_id}"
            )
            ...
            logger.info(
                f"Template Manager Agent: Feedback processing completed for template {template_id}"
            )
    ...
    async def create_new_template(
        ...
        try:
            logger.info(
                f"Template Manager Agent: Creating new template '{name}'"
            )
            ...
            logger.info(
                f"Template Manager Agent: Template creation completed for '{name}'"
            )
...
if __name__ == "__main__":
    ...
    async def test_template_manager():
        ...
        print(
            f"Feedback processing result: {result['status']}"
        )
        ...
        print(
            f"Template creation result: {creation_result['status']}"
        )
