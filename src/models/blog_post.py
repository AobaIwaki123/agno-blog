...
class BlogPostBase(BaseModel):
    ...
    content: str = Field(..., min_length=10, description="Main blog post content")
    url_source: Optional[str] = Field(None, description="Source URL if generated from URL")
    template_used: str = Field(default="default", description="Template ID used for generation")
...
class BlogPost(BlogPostBase):
    ...
    id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Unique blog post ID")
    ...
    word_count: int = Field(default=0, description="Word count of the content")
    reading_time: int = Field(default=1, description="Estimated reading time in minutes")
    ...
    def calculate_word_count(cls, v, values):
        ...
        if 'content' in values:
            import re
            words = re.findall(r'\b\w+\b', values['content'])
            return len(words)
        return v
...
class BlogTemplateBase(BaseModel):
    ...
    content: str = Field(..., min_length=10, description="Template content with placeholders")
...
class BlogTemplate(BlogTemplateBase):
    ...
    id: str = Field(default_factory=lambda: str(uuid.uuid4())[:8], description="Unique template ID")
    version: str = Field(default="1.0.0", description="Template version")
    created_at: datetime = Field(default_factory=datetime.utcnow, description="Creation timestamp")
    updated_at: datetime = Field(default_factory=datetime.utcnow, description="Last update timestamp")
    usage_count: int = Field(default=0, description="Number of times template was used")
    feedback_score: float = Field(default=5.0, ge=0.0, le=10.0, description="Average feedback score")
...
class BlogPostDB(Base):
    ...
    template_used = Column(String(50), nullable=False, default="default")
    tags = Column(JSON, default=list)  # Store as JSON array
    metadata = Column(JSON, default=dict)  # Store as JSON object
...
class BlogTemplateDB(Base):
    ...
    feedback_history = Column(JSON, default=list)  # Store as JSON array
...
# Utility functions
def create_blog_post_from_content(title: str, content: str, url_source: str = None,
                                template_id: str = "default") -> BlogPost:
    """
    ...
    common_tech_terms = [
        "python", "javascript", "react", "nodejs", "api", "database",
        "web development", "programming", "tutorial", "guide", "tips"
    ]
    
    tags = [term for term in common_tech_terms if term in combined_text][:5]
    ...
    return BlogPost(
        ...
        metadata={
            ...
            "source_type": "url" if url_source else "manual"
        }
    )
...
*Tags: {tags}*
*Source: {source_url}*""",
        variables=["title", "introduction", "main_content", "key_points", "conclusion", "publish_date", "tags", "source_url"]
    )
...
*Tags: {tags}*""",
        variables=["title", "overview", "prerequisites", "steps", "language", "code_example", 
                  "troubleshooting", "conclusion", "resources", "difficulty_level", "estimated_time", "tags"]
    )
...
*Source: {source_url}*
*Tags: {tags}*""",
        variables=["title", "publish_date", "author", "summary", "main_content", "impact", 
                  "future_implications", "related_links", "source_url", "tags"]
    )
